...
const LibraryPage: React.FC = () => {
  ...
  const [selectedSurah, setSelectedSurah] = useState<SurahName>('Al-Fatiha');
  ...
  // Video resources for each Surah - typed record
  const surahVideos: Record<SurahName, Video[]> = {
    'Al-Fatiha': [
      {
        id: 1,
        title: 'Introduction to Surah Al-Fatiha',
  ...
  // Get videos for the selected Surah
  const currentSurahVideos: Video[] = selectedCategory === 'Quran' ? surahVideos[selectedSurah] || [] : [];
                      ...
                      <div key={video.id} className="video-card">
                        ...
                        <div className="video-content">
                          ...
                          <div className="video-meta">
                            <span><i className="fas fa-clock" /> {video.duration}</span>
                            <span><i className="fas fa-eye" /> {video.views}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                {filteredResources.map((res) => (
                  <div key={res.id} className="resource-card">
                    <div className={`resource-header ${getResourceTypeClass(res.type)}`}>
                      ...
                      <div className="resource-category">{res.type.charAt(0).toUpperCase() + res.type.slice(1)}</div>
                    </div>
